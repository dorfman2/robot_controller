...
## Upcoming Changes
- **RESUME HERE (vision): reachability probe** — map the reachable low-Z region so we know if/where the arm can reach the desk plane (z≈78 mm). Probe `move_cartesian` at closer-in X (-150/-180/-210) and descending Z. The extended-X points can't reach down (joint-limit clamp). This gates the whole pick. May need to move the rail to position the reachable patch over the pen.
...
- **Motor current (validated, all sense_resistor 0.050)**: rail(x) 0.8/0.3 · base(y) 0.8/0.3 · shoulder(z) **1.5/1.1** · elbow(a) **1.2/1.0** · J3 pitch(b) 1.0/1.0 · roll(c) 1.0/1.0 · yaw(u) 1.0/1.0 (run/hold A). Raised most joints to 1.0/1.0 for firmer hold; J2 exception 1.2 run; J1 shoulder 1.4/1.1 (lifts 90° at full extension, idles ~32°C). Rail left 0.8/0.3.
