...
## Current Focus
**FK/IK MODEL FULLY CORRECTED & DEPLOYED (2026-08-02).** The old model was badly wrong (link lengths, tool orientation, AND 5 of 6 joint sign directions). Rebuilt `ik_solver.py` from in-situ measurements + a full joint-direction check on the hardware. All 15 ik tests pass, lint clean, deployed. NEXT: re-home to resync the daemon (deploy reset it to 0), then verify `move_cartesian` drives the arm where the corrected model predicts (small moves), then resume the descent/touch-test + hand-eye + pen pick.
