...
- **OAK-4 S is monocular (NO stereo depth)** — like the OAK-1 Lite, depth must come from a known plane (desk homography) or the side camera, not on-board stereo. RVC4 platform.
- **RVC4 on-device NN uses `nn_archive` (.tar), NOT `.blob`** (blob = Myriad/RVC2 only). Zoo slugs (e.g. `yolov6-nano`) load directly; custom models come from HubAI ModelConverter as an nn_archive. yolov6-nano runs ~29 FPS on the OAK-4 S with the Pi receiving only the decoded detections (no image transfer) — the win of on-device inference.
- **COCO models have no `pen` class** — a zoo NN can't drive a pen pick. Either train a custom class (Roboflow→ultralytics→HubAI→nn_archive) or keep classic-CV pen detection. Classic CV drives the pick meanwhile.
- **Auto-labeling as weak supervision**: bootstrap YOLO labels by running the existing classic-CV detector over the capture set (`autolabel_pen.py`), emitting normalized `<class> <cx> <cy> <w> <h>` (empty file = valid background sample). This cuts annotation effort but MUST be human-reviewed — a model trained on unreviewed auto-labels just mimics the classic detector (errors included).
- **Gate auto-labels by detector confidence**: the classic pen detector's confidence cleanly separates tight-correct boxes (380+) from partial/edge failures (~120). A `--min-confidence` threshold (~200) converts weak detections to background rather than emitting a wrong box — big quality lift for near-zero effort. Store no confidence in the YOLO label itself, so filter at emit time.
- **Classic-CV pen detector rejects clutter well**: across capture frames, `detect_pen` (flat-field bg-subtract + Otsu + elongation) boxed pens of varying color tightly and did NOT false-positive on scissors, pliers, brush, sandpaper, or baggies — the elongation + ROI gate is doing real work. Main failure = pens near/outside the ROI edge (partial box, low conf) and hand-occluded pens (miss).
- **Nested heredocs over `ssh 'bash -s' <<'EOF'` hang** — an inner `cat > f <<'YAML' … YAML` inside the outer heredoc stalls the remote shell. Create config files locally with the editor and `scp` them, or write each with a single non-nested command. Keep `tar`/packaging as its own one-line ssh command.
- **Review-ready YOLO dataset layout**: `images/` + `labels/` (one .txt per image, empty=background) + `data.yaml` (`nc`, `names`) + `previews/` + `README`. Build/tar it ON the Pi (where the images live) then retrieve; keep the tarball gitignored (120 frames ≈ 122 MB).
...
- **macOS crypt(3) is DES-only** (`crypt.methods == [METHOD_CRYPT]`): `crypt.crypt()` with a `$6$` salt silently returns DES garbage, and macOS LibreSSL has no `openssl passwd -6`. Generate SHA-512-crypt hashes with **passlib** (`sha512_crypt.hash()`, pinned 1.7.4 in the Mac venv) and verify round-trip before shipping.
- **Headless Pi image flow (proven 2026-08-15)**: download `ubuntu-24.04.x-preinstalled-server-arm64+raspi.img.xz` + verify SHA256 → user runs `xz -dc … | sudo dd of=/dev/rdiskN bs=4m` (agent can't sudo; raw disk is root:operator) → remount `system-boot` (FAT, user-writable) → overwrite `user-data` with the cloud-init seed → eject. Seed with packages+groups+keys does most of Phase-B provisioning itself. Validate the seed with yaml.safe_load BEFORE flashing.
- **Verify the passwd line, not a substring**: `'passwd' in line` also matches `lock_passwd` — a buggy verifier reported a good hash as bad twice. Match `line.strip().startswith("passwd: ")`.
- **opencv-python on headless Ubuntu server needs `libgl1`** (+`libglib2.0-0t64` on Noble) — `ImportError: libGL.so.1` otherwise.
- **After a Pi reflash**: `ssh-keygen -R` armold.local AND 192.168.1.136 AND armold (three known_hosts entries), else host-key mismatch warnings.
- **udev rule for the rail Mega**: genuine Mega 2560 = VID 2341 PID 0042 → `/dev/armold_rail` (CH340 clones = 1a86:7523). Rules live at `/etc/udev/rules.d/99-armold.rules` (recreate from pi/REPROVISION.md Phase C).
- **UIM4247PM (UIM344 family) ground truth** (from `Manual_UIM344 V5.11.pdf` in repo root): 9-terminal single-ended interface, opto common-ANODE (COM→5V) → STP/DIR/ENA are ACTIVE-LOW; ENA must be driven LOW to enable (floating = disabled); min pulse width >3 µs high+low (use 5/5 µs); default microstepping 32 (6400 pulses/rev); UART config (t8/t9) is TTL @ **57600** (extracted from CFG344.exe — the only baud constant), protocol undocumented, vendor CFG344 Windows tool does one-time setup. Canonical Mega pin map: STP=D2, DIR=D3, ENA=D4, endstop=D5 (NC↔GND, INPUT_PULLUP: HIGH=triggered/wire-break = fail-safe, verified passing).
- **CANBUS Stepper vendor doc has typos in its worked frame examples**: "90.0" payload bytes actually encode 88.0, and the node-2 angle example says ID 0x91 but (2<<6)|33 = 0xA1. Trust IEEE-754 + firmware source; `armold_can_driver/test/test_protocol.py` documents both.
- **PD-Stepper repo (joshr120) is the wireless reference** for CANBUS-Stepper-class boards (WiFi web server, ESPHome, ESP-NOW firmware). Decision: no radio on motion nodes — the Pi is the wireless gateway (option 1).
- **CANBUS Stepper flash/provision recipe (proven on all 6 boards, 2026-08-16)**: `esptool --port /dev/armold_can_bridge write_flash 0x0 <full-8MB image>` (vendor GUI flashes at 0x0, `flashSize: keep`); backup first with `read_flash 0x0 0x800000` at DEFAULT baud (921600 corrupts on S3 USB-JTAG). ESP32-S3 node enumerates as VID 303a PID 1001 (USB-Serial-JTAG) → udev `/dev/armold_can_bridge`.
- **Full-image flash WIPES NVS** → NodeID reverts to default 1: park the USB-attached node at a temp ID (e.g. 7) first so fresh boards don't collide; SET_NODE_ID (25) has NO bus-side guard — a frame targeted at ID 1 changes EVERY ID-1 node on the bus. Save (24) must be addressed to the node's NEW id after renaming.
- **⚠ CLOSED-LOOP RUNAWAY (J1 incident 2026-08-16)**: enabling closed loop (MsgType 13) with the direction map (15) not matched to the motor loom (ABAB vs ABBA) makes the loop positive-feedback — motor ran away at ~2000 deg/s until stalled on the wound cable. NEVER enable closed loop before the per-joint direction-sense check (jog +10° open-loop, verify encoder delta sign, set 15, then 13). Provisioning leaves boards OPEN-LOOP; stock default enable-on-boot=ON — send disable FIRST on a fresh board.
- **XT30 (2+2) gotcha**: power pins can mate while CAN pins don't — a green LED does NOT mean the node is on the bus. Also **CAN visibility can be position-dependent when marginal** (nodes 2/5 heard from J2 but not from J1): suspect latched **bus-off** (flaky contact → TX error accumulation → controller off-bus until reboot) or wrong termination; a node's OWN USB echoes its local telemetry even when the bus never hears it — don't treat local echo as bus proof.
- **Sweep Dynamics firmware analysis (from J1 8MB backup, NVS namespace parse)**: J1 was the **WiFi/cloud gateway** (AP+STA creds, sweepsync robot UUID, ~28KB cloud credential blob, per-fleet `s2..s6HomeAngle` keys — all 0.0°); nodes 2–6 were CAN-only followers. Motor config: **microsteps=16** (matches Armold 230.6 steps/deg constants), maxCurrent=30 (~0.58 A), stallThresh=10, stepsPerRev=200. **No direction-map key stored** — Sweep handled direction in code, so the per-joint direction check cannot be skipped. Sweep firmware IGNORES the stock command set (RTR, report-freq, LED all unanswered) — reflash was the only path.
- **NVS parsing pattern**: partition table at 0x8000 (`AA 50` entries) → nvs partition (type 1 sub 0x02, typically 0x9000+0x5000); 4096-B pages, 32-B entries, key at bytes 8–24, small ints inline at 24–32, floats/strings as follow-on BLOB entries. Arduino Preferences namespace appears as ns=1.
...
- **Pi 5 (2 GB, Ubuntu 24.04, arm64)**: armold_controller daemon + vision services + ROS 2 Jazzy (reflashed 2026-08-15; was Pi 4)
